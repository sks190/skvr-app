...
declare
    %templates:wrap
    %templates:default("sort", "idno")
function app:list-works($node as node(), $model as map(*), $path as xs:string?, $filter as xs:string?, $browse as xs:string?, $odd as xs:string?, $sort as xs:string) {
    ...
    let $filtered :=
        if (app:use-cache($params, $cached)) then
            ...
            query:query-metadata($path, $browse, $filter, $sort)
        else
    ...
    return (
        ...
        if (empty($cached)) then
            session:set-attribute($config:session-prefix || ".collection", $path)
        else
            (),
        map {
            "all" : $sorted,
