...
declare function sapi:search($request as map(*)) {
    ...
    then
        ...
        let $hitsAll :=
                ...
                let $dateSort := request:get-parameter("dateSort","")                
                let $sorted := for $hit in query:query-default($request?parameters?coll, $request?parameters?field, $request?parameters?query, $request?parameters?doc, ())
                    order by
                        if ($dateSort) then $hit/ancestor::tei:TEI//tei:setting/tei:date[@type="standardized"] else util:document-name($hit) ascending
                    return $hit
                let $sortedAgain := for $sortedHit in $sorted
                    order by util:collection-name($sortedHit) descending
                    return $sortedHit
                return $sortedAgain
        ...
        let $store := (
            ...
            session:set-attribute($config:session-prefix || ".coll", $request?parameters?coll)
        )
        return
            if ($hitCount > 0) then 
                sapi:show-hits($request, $hits, $request?parameters?doc)
            else
                <p style="font-size: 18px; color: black;">Tuloksia ei löytynyt</p>               
};
...
declare %private function sapi:show-hits($request as map(*), $hits as item()*, $docs as xs:string*) {
    ...
    return
        <paper-card>
            ...
            <div class="matches">
            {
                for $match in subsequence($expanded//exist:match, 1, 80000)
                ...
                let $docLink :=
                    if ($config?view = "page") then
                        ...
                        util:node-id($div)
                let $config := <config width="100" table="no" link="{$docId}?action=search&amp;odd={$config?odd}#{$matchId}"/>
                return
...
declare function sapi:list-facets($request as map(*)) {
    ...
    let $matches := 
        ...
        order by $label collation "http://www.w3.org/2013/collation/UCA?numeric=yes;lang=fi"
        return 
            map {
                "text": $label,
                "freq": $facets($key),
